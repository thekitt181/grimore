...
export function markCompendiumWritePending() {
    lastNotifiedMs = Date.now();
}
export function startCompendiumMongoWatch() {
    if (started || !isMongoConfigured())
        return;
    started = true;
    void (async () => {
        ...
        const watch = () => {
            try {
                const stream = col.watch([{ $match: { 'documentKey._id': 'global' } }], { fullDocument: 'updateLookup' });
                stream.on('change', (change) => {
                    const now = Date.now();
                    if (now - lastNotifiedMs < SELF_ECHO_MS)
                        return;
...
//# sourceMappingURL=compendiumMongoWatch.js.map