...
import { isCompendiumStorageUnavailable, updateCompendiumPolicyFields } from './compendiumPostgres';
...
import { collectImportedSourceLabelListFromMongo } from './compendiumMongoReads';
...
export async function collectImportedSourceLabels(): Promise<string[]> {
  const labels = new Set<string>();
  for (const label of await collectImportedSourceLabelListFromMongo()) {
    labels.add(label);
  }
...
export async function ensureBundledSourcesLocked(reason: string): Promise<number> {
  return enqueueCompendiumWrite(async () => {
    ...
    const col = !isCompendiumStorageUnavailable();
    if (col) {
      ...
      try {
        ...
        console.warn(
          '[Compendium] Postgres bundled-lock write failed, using fallback:',
  ...
  return enqueueCompendiumWrite(async () => {
    ...
    if (!isCompendiumStorageUnavailable()) {
      ...
      try {
        ...
        console.warn(
          '[Compendium] Postgres imported-unlock write failed, using fallback:',
          err instanceof Error ? err.message : err,
        );
      }
    }
