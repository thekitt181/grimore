...
function matchesRemovedSource(source) {
  if (!source) return false;
  const lower = String(source).toLowerCase();
  return lower.includes('monster manual') || lower.includes('tome of beasts');
}
...
function filterList(list) {
  ...
  for (const entry of list) {
    if (matchesRemovedSource(entry?.source)) removed.push(entry);
...
function cleanMonsterFiles() {
  ...
  for (const filePath of paths) {
    ...
    const kept = raw.filter((m) => !matchesRemovedSource(m.source));
    const removed = raw.length - kept.length;
    if (removed > 0) {
      fs.copyFileSync(filePath, `${filePath}.bak-before-mm-tob-removal`);
...
async function cleanMongo() {
  ...
  const ids = all.filter((m) => matchesRemovedSource(m.source)).map((m) => m._id);
  if (ids.length) {
