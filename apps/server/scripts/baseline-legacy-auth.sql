-- One-time legacy fix: Supabase DBs created before Prisma migrate history.
-- Renames clerkId → authUserId in place (preserves existing User rows).
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'auth_session_userId_fkey'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'auth_account_userId_fkey'
  ) THEN
