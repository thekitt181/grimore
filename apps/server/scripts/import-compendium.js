...
async function main() {
    ...
    const monstersRaw = JSON.parse(fs.readFileSync(path.join(dir, 'monsters.json'), 'utf8'));
    const itemsRaw = JSON.parse(fs.readFileSync(path.join(dir, 'items.json'), 'utf8'));
    const spellsRaw = JSON.parse(fs.readFileSync(path.join(dir, 'spells.json'), 'utf8'));
    ...
    const spellOps = Object.entries(spellsRaw).map(([key, val]) => {
        ...
        const spell = { name, level: val.level, ...val };
        return {
            updateOne: {
                filter: { _id },
    ...
    if (monsterOps.length)
        await db.collection('monsters').bulkWrite(monsterOps, { ordered: false });
    if (itemOps.length)
        await db.collection('items').bulkWrite(itemOps, { ordered: false });
    if (spellOps.length)
        await db.collection('spells').bulkWrite(spellOps, { ordered: false });
    ...
    await db.collection('data').updateOne({ _id: 'global' }, {
...
//# sourceMappingURL=import-compendium.js.map